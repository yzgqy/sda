...
//        System.out.println(taskEntity);
        ...
        TaskInputDataDto taskInputDataDto = taskEntity.getInputDataDto();
        InputData inputData = taskInputDataDto.toInputData();
    ...
    @Override
    public PageQueryDto<TaskEntity> queryTaskInfoCollection(TaskEntity taskEntity, PageQueryDto<TaskEntity> dto) {
        Page page = PageHelper.startPage(dto.getPageNum(), dto.getPageSize(), true);

        Example example = new Example(TaskEntity.class);
        example.createCriteria().andEqualTo("appId",taskEntity.getAppId())
        .andEqualTo("type",taskEntity.getType()).andNotEqualTo("status","Error");
        example.setOrderByClause("created_at desc");
        List<TaskEntity> taskEntities = taskMapper.selectByExample(example);
        for(TaskEntity te:taskEntities){
            List<TaskDataEntity> taskDataEntities = queryTaskDataEntityByTaskId(te.getId());
            te.setTaskDataList(taskDataEntities);
        }
