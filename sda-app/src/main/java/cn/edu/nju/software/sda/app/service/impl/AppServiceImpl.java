...
*/
    ...
    public PageQueryDto<AppEntity> queryAppListByLikePaged(@NotNull PageQueryDto<AppEntity> dto, AppEntity app) {
        ...
        if(app.getUserId()!=null)
            criteria.andEqualTo("userId",app.getUserId());
        if(app.getIsStarred()!=null&&app.getIsStarred()==1)
            criteria.andEqualTo("isStarred",1);

        if(app.getIsStarred()!=null&&app.getIsArchived()==1)
            criteria.andEqualTo("isArchived",1);
        else
            criteria.andEqualTo("isArchived",0);
