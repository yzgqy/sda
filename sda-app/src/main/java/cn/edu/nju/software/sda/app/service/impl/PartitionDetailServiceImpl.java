...
//        builder.attachOperate(addNodeToPartition(nodeId, targetPartitionNodeEntity));
    ...
    @Override
    @Transactional(propagation = Propagation.REQUIRED)
    public PartitionGraphOperateDto addNodeToPartition(List<String> nodeIds, PartitionNodeEntity targetPartitionNodeEntity) {
        PartitionGraphOperateDto.Builder builder = new PartitionGraphOperateDto.Builder();
        Set<String> set = new HashSet<>(nodeIds);
        /**
         * 简单的方案：调整节点所属划分，重新计算边的关系
        ...
        for (String nodeId :
                set) {
            PartitionDetailEntity pd = new PartitionDetailEntity();
            pd.setId(Sid.nextShort());
            pd.setNodeId(nodeId);
            pd.setPartitionNodeId(targetPartitionNodeEntity.getId());
            pd.setUpdatedAt(new Date());
            pd.setCreatedAt(new Date());
            pd.setFlag(1);
            partitionDetailMapper.insert(pd);
        }


        partitionNodeEdgeService.statisticsPartitionResultEdge(targetPartitionNodeEntity.getPartitionId());
        builder.graphReload();
