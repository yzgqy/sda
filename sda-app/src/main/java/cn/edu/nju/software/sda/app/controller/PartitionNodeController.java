...
public class PartitionNodeController {
    ...
    @Autowired
    private ExtraNodeService extraNodeService;
    ...
    //查询拆分未包含的结点
    @RequestMapping(value = "/partition/extranode", method = RequestMethod.GET)
    public JSONResult partitionExtraNode(String appId ,String partitionId) throws Exception {
        List<Node> nodes = extraNodeService.findExtraNode(appId,partitionId);
    ...
    @RequestMapping(value = "/partition-results/addNode/{partitionInfoId}", method = RequestMethod.POST)
    public JSONResult partitionAddResultDetailNodes(@PathVariable String partitionInfoId, @RequestBody List<String> nodeIds, String targetPartitionResultName) {

        PartitionGraphOperateDto dto = partitionDetailService.addNodeToPartition(nodeIds,
                partitionNodeService.queryPartitionResult(partitionInfoId, targetPartitionResultName));
