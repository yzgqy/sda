...
public class AppController {
    ...
    @RequestMapping(value = "/app/apps", method = RequestMethod.GET)
    public ResponseEntity queryApp(@RequestParam(defaultValue = "1")Integer pageNum, @RequestParam(defaultValue = "10")Integer pageSize, String name, String desc,String isStarred,String isArchived,String userId) {
        name = StringUtils.stripToNull(name);
        desc = StringUtils.stripToNull(desc);
        userId = StringUtils.stripToNull(userId);
        isStarred = StringUtils.stripToNull(isStarred);
        isArchived = StringUtils.stripToNull(isArchived);
        int starred = 2;
        int archived = 2;
        if(isStarred!=null)
            starred = Integer.valueOf(isStarred);
        if(isArchived!=null)
            archived = Integer.valueOf(isArchived);
        AppEntity appEntity = AppEntity.createByNameAndDesc(name, desc);
        appEntity.setIsArchived(archived);
        appEntity.setIsStarred(starred);
